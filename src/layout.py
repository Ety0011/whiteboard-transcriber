...
_global_detector: LayoutDetector | None = None
...
def init(
    ...
    _global_detector = LayoutDetector(confidence_threshold, recompute_interval)


def process(image: np.ndarray) -> list[Region]:
    """Detect layout regions using the module-level singleton detector.
    ...
    Returns:
        Last cached list of Region objects (empty until first detection done).
    """
    global _global_detector
    if _global_detector is None:
        log.warning("layout.process() called before init() — using defaults")
        _global_detector = LayoutDetector()
    return _global_detector.process(image)
