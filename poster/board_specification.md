...
On the whiteboard, handwritten in black marker, are five text blocks in different positions spread naturally across the surface. Each block is outlined by a solid colored rectangle — a CV bounding box overlay drawn by software, not by hand. Each rectangle has a small label tag in the top-left corner in the same color as the box, using a monospace font, showing the entity ID and state name.

**Bounding box style (applies to all blocks):** Each box has a solid colored border (thickness 2px) and a translucent fill of the same color at ~25% opacity. The fill tints the whiteboard surface inside the box; the handwritten text remains visible through it.
...
- Then wrote "Forward pass: make a prediction", originally had "Forward pass: make a guess" — corrected in place (Block 2, ACTIVE, 1 revision)
